            ...
            {coworking && (
              <p>
                <span className="flex items-center gap-1.5 font-semibold text-primary">
                  <span className="material-symbols-outlined text-secondary" style={{ fontSize: 16 }}>domain</span>
                  {coworking.nom} · {coworking.ville}
                </span>
              </p>
            )}
  ...
  return (
    <div className="min-h-screen bg-surface p-4 sm:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        {/* Étape 1 — Choix coworking + espace + créneau */}
        {step === 1 && (
          <div>
            {/* Sélecteur de coworking (si pas de tenantId) */}
            {!coworking && (
              <div className="bg-white rounded-3xl border border-outline-variant/20 p-6 shadow-sm mb-5">
                <h2 className="font-sora font-bold text-primary text-base mb-1">Choisissez un coworking</h2>
                <p className="text-xs text-on-surface-variant mb-4">Les espaces partenaires actifs apparaissent ici avec leurs coordonnées.</p>
                {coworkings.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {coworkings.map((c) => (
                      <button key={c.id} onClick={() => selectCoworking(c)}
                        className="p-3 rounded-2xl border text-left transition-all hover:border-secondary/30 hover:shadow-sm overflow-hidden"
                        style={{ borderColor: 'rgba(0,0,0,0.1)' }}>
                        <img src={coworkingImg(c)} alt={c.nom}
                          className="w-full h-24 object-cover rounded-xl mb-2.5" />
                        <p className="font-semibold text-primary text-sm">{c.nom}</p>
                        ...
                        {c.latitude && c.longitude ? (
                          <p className="text-[10px] text-on-surface-variant/80 mt-0.5">
                            📍 {c.latitude}, {c.longitude} · {c.space_count ?? 0} espaces
                          </p>
                        ) : (
                          <p className="text-[10px] text-on-surface-variant/80 mt-0.5">{c.space_count ?? 0} espace(s) disponibles</p>
                        )}
                      </button>
                    ))}
                  </div>
            ...
            {/* Carte du coworking sélectionné (image + coordonnées) */}
            {coworking && (
              <div className="bg-white rounded-3xl border border-outline-variant/20 p-5 shadow-sm mb-4">
                <div className="flex items-start gap-4 flex-wrap">
                  <img src={coworkingImg(coworking)} alt={coworking.nom}
                    className="w-full sm:w-56 h-32 object-cover rounded-2xl" />
                  <div className="flex-1 min-w-[220px]">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h2 className="font-sora font-bold text-primary text-base">{coworking.nom}</h2>
                        <p className="text-xs text-on-surface-variant mt-0.5 flex items-center gap-1">
                          <span className="material-symbols-outlined" style={{ fontSize: 14 }}>location_on</span>
                          {coworkingPlace(coworking) || 'Tunisie'}
                        </p>
                      </div>
                      <button onClick={resetCoworking}
                        className="text-xs font-semibold text-on-surface-variant hover:text-primary flex items-center gap-1">
                        <span className="material-symbols-outlined" style={{ fontSize: 14 }}>edit</span>
                        Changer
                      </button>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 mt-2 text-xs text-on-surface-variant">
                      {coworking.telephone && (
                      ...
                      {coworking.description && (
                        <p className="sm:col-span-2 mt-1 line-clamp-2">{coworking.description}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-3xl border border-outline-variant/20 p-6 shadow-sm">
              ...
              {!coworking ? (
                <p className="text-sm text-on-surface-variant">Sélectionnez d'abord un coworking ci-dessus.</p>
              ) : espaces.length === 0 ? (
                <div className="text-center py-6 text-sm text-on-surface-variant">
                  Aucun espace publié dans cet établissement pour le moment.
                </div>
              ) : (
        ...
        {step === 2 && (
          <div className="bg-white rounded-3xl border border-outline-variant/20 p-6 shadow-sm">
            ...
            {/* Récap espace */}
            <div className="bg-secondary/5 border border-secondary/15 rounded-2xl p-3 mb-5">
              <div className="flex items-start gap-3">
                <span className="material-symbols-outlined text-secondary" style={{ fontSize: 20 }}>location_on</span>
                <div className="flex-1">
                  <p className="font-semibold text-primary text-sm">
                    {selected?.nom}
                    {coworking ? <span className="text-on-surface-variant font-normal"> — {coworking.nom}</span> : null}
                  </p>
                  <p className="text-xs text-on-surface-variant">
                    {coworking ? `${coworkingPlace(coworking)} · ` : ''}
