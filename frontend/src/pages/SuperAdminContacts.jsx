...
export default function SuperAdminContacts({ session }) {
    ...
    <PortalLayout profile={profile}>
      <div className="p-2 sm:p-6 lg:p-8 max-w-[1400px] mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4 mb-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-on-surface-variant">Super Admin</p>
            <h1 className="font-sora text-xl sm:text-2xl font-bold">Contacts reçus</h1>
          </div>
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && loadContacts()} placeholder="Rechercher..." className="rounded-xl border border-outline-variant/30 px-3 py-2 text-sm w-full sm:w-auto" />
            <select value={status} onChange={e => setStatus(e.target.value)} className="rounded-xl border border-outline-variant/30 px-3 py-2 text-sm w-full sm:w-auto"><option value="">Tous</option><option value="nouveau">Nouveaux</option><option value="lu">Lus</option><option value="traite">Traités</option></select>
          </div>
        </div>
